...
module Language.Lexer.Applicative.Text
  (
    -- * Building a Lexer
    Lexer(..)
  , token
...
-- error is signaled.
newtype Recognizer tok = Recognizer (RE Text (RE Text tok))
  deriving Functor
...
-- @
longestShortest
  :: RE Text pref -- ^ regex for the longest prefix
  -> (pref -> RE Text tok) -- ^ regex for the shortest suffix
  -> Recognizer tok
...
runLexer (Lexer (Recognizer pToken) (Recognizer pJunk)) src = go . annotate src
  ...
  extend :: RE Text a -> RE (Text, Pos, Pos) a
  extend = comap (\(c, _, _) -> c)

  re :: RE (Text, Pos, Pos) (RE (Text, Pos, Pos) (Maybe tok))
...
annotate
  ...
  -> Text -- ^ contents
  -> [(Text, Pos, Pos)] -- ^ the character, its position, and the previous position
annotate src s = snd $ mapAccumL f (startPos src, startPos src) $ Text.unpack s
  where
    f (pos, prev_pos) ch =
      let pos' = advancePos pos ch
      in pos' `seq` ((pos', pos), (Text.singleton ch, pos, prev_pos))
